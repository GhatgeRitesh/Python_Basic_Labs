...
from mymodule import dubler , square
class TestMyModule(unittest.TestCase): # TestCase is class from the unittest library to inherite the class u=you can use .method 
     
    def test_square(self):   # only functions which starts with test_ will run the test cases 
        # always give the function name starting with test_
        self.assertEqual(square(2),4)  # add one or more unit assert methods to check the unit test cases are met
        # assertEqual is just one method there are many 
        print("test is done1")

    def test_dubler(self):
        self.assertEqual(dubler(0),0)
        print("test is done2")

if __name__ == '__main__': # method has been added to the test case class into the code
    unittest.main()      

# asserEqual method compare the two values are equal or not 
# the method is to check the methods are returning the correct values 
# syntax assertEqual(actual_value , expected_value)

# after running the this test file output is generated 
# output displays some addtional results and items 
# returns time and status may vary all of these



# course code  used into the labs 
